...
# linear regression using hchart ###take out 
model = lm(ABV ~ IBU, data = combined_df_nB)
...
#delete NA for IBU into BrewBeer2 = we can remove these lines 
...
# get column names
dfColumns = data.frame(colnames(ipas))
dfColumns
...
classifications = knn(train[,c(8,9)],test[,c(8,9)],as.factor(train2$ipasType),prob=TRUE,k=5)
...
classifications4 = knn(train[,c(7,8)],test[,c(7,8)],as.factor(train$ipasType),prob=TRUE,k=75)
CM4 = confusionMatrix(table(as.factor(test$ipasType),classifications4))
CM4

classificationsImp = knn(train[,c(7,8)],test[,c(7,8)],as.factor(train$ipasType),prob=TRUE,k=i)
CMImp = confusionMatrix(table(as.factor(test$ipasType),classificationsImp))
CMImp
```
