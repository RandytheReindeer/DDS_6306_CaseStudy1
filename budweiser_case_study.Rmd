...
#load libraries
```{r library, include=T, error=F, message=F, results='hide'}
library(tidyverse)
library(ggthemes)
library(dplyr)
library(ggmap)
...
# Breweries by state 
# BreweriesByState = data.frame(table(breweries$State, breweries$Region))
# colnames(BreweriesByState) = c("State","Region","Breweries")
# # removed from knit file = doesn't display correctly
#  BreweriesByState 
...
```
The ABV variable had `r zeros` imputed zeros. The error rate for this is `r zeros/length(combined_df_nB$ABV)*100`%. Additionally, 75% of the ABV falls between 5% - 6.7%. A general note of interest is that Budweiser has an ABV of 5%.
...
confusionMatrix(table(classifications, as.factor(ipas$ipasType)))
# Accuracy = 81.8% 
# Sensitivity = 85.4%
# Specificity = 75.7%

```
From external cross-validations, the optimal number of nearest neighbors is `r which.max(meanAcc)`. From this, we checked our accuracy, sensitivity, and specificity of predicting IPA versus non-IPA ales. The accuracy for predicting IPA with internal cross-validation based on ABV and IBU was found to be `r round(CM$overall[[1]],4)`, the specificity was `r round(CM$byClass[[2]],4)`, and the sensitivity was `r round(CM$byClass[[1]],4)`.
