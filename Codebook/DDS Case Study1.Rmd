---
title: "DDS Case Study 01"
author: "Randy Kim  Kati Schuerger  Will Sherman"
date: "6/25/2021"
output:
...
#BreweriesByState$State = substr(dfBrews$State, 2,3)
str(breweries)
...
# attaching the predictions for unknown to know IBUs 
...
```{r}
# ABV qantiles for median ABV values 
...
- 75% of the ABV falls between 5% - 6.7%.
- Budweiser is 5%
...
ipa = filter(combined_df_nB, grepl("Ale|IPA",Style))
ipa = ipa %>% mutate(ipaType = ifelse(grepl("IPA",Style),"IPA","Ale"))
ipa %>% ggplot(aes(x=ABV *100, y= IBU, color=ipaType)) +
  geom_point(position='jitter') +
  geom_smooth() +
  ggtitle('ABV vs IBU') +
  labs(subtitle="Ale and ipa")+
  xlab('ABV (%)') +
  theme(legend.title=element_blank())
...
for (j in 1:iterations)
{
  ...
  for (i in 1:nums)
  {
    classifications = knn(train[,c(8,9)],test[,c(8,9)],as.factor(train$ipaType),prob=TRUE,k=i)
    CM = confusionMatrix(table(as.factor(test$ipaType),classifications))
...
classifications = knn.cv(ipa[,c(8,9)], ipa$ipaType, prob = TRUE, k = 38)
confusionMatrix(table(classifications, as.factor(ipa$ipaType)))
# Accuracy = 81.8% 
# Sensitivity = 85.4%
# Specificity = 75.7%
```
